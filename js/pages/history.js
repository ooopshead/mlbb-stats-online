import * as store from '../store.js'; // Для списка героев
...
export function initHistoryPage() {
    ...
    let allMatches = []; // Кэшируем матчи на странице
    ...
    const openEditModal = (matchId) => {
        ...
        dataService.getPatches().forEach(p => patchSelect.add(new Option(p, p)));
        ...
        if (deleteBtn) {
            ...
            ui.showConfirm('Вы уверены, что хотите удалить этот матч?', async () => {
                ...
                if (success) {
                    ...
                    await loadPageData(); // Перезагружаем данные
                } else {
    ...
    editForm.addEventListener('submit', async (e) => {
        ...
        const updatedData = {
            ...originalMatch, // Сохраняем id и date
    ...
    importInput.addEventListener('change', (event) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (!importedData || !Array.isArray(importedData.matches)) {
                    throw new Error("Некорректный формат файла: отсутствует массив 'matches'.");
                }
                
                const matchesToImport = importedData.matches;

                ui.showConfirm(`Вы уверены, что хотите импортировать ${matchesToImport.length} матчей? Это действие нельзя отменить.`, async () => {
                    ui.showToast('Начинаем импорт. Это может занять некоторое время...', 'info');
                    
                    const result = await dataService.importMatches(matchesToImport);

                    if (result.success) {
                        ui.showToast(`Успешно импортировано ~${result.count} матчей!`, 'success');
                        await loadPageData(); // Обновляем данные на странице
                    } else {
